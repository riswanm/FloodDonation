...
3. Create two sheets (tabs) with the following names:
   - `Bookings` - For storing booking requests
   - `Config` - For storing configuration like issued shares count
...
### Step 3: Setup the Config Sheet

In the `Config` sheet, add the following:

| A | B |
|---|---|
| issuedShares | 0 |

- Cell A1: `issuedShares`
- Cell B1: The number of shares that have been issued (start with `0` or your current count)

**Important:** Update cell B1 whenever shares are confirmed and paid for.
...
/**
 * Get the number of issued shares from Config sheet
 */
function getIssuedShares() {
  ...
  // Read issued shares from B1 (assuming A1 is label, B1 is value)
  const issuedShares = configSheet.getRange('B1').getValue();
  
  return ContentService.createTextOutput(JSON.stringify({
    success: true,
    issuedShares: parseInt(issuedShares) || 0
  })).setMimeType(ContentService.MimeType.JSON);
}
...
## Managing Issued Shares

To update the number of issued shares:

1. Open your Google Sheet
2. Go to the `Config` sheet
3. Update the value in cell **B1** with the new total of confirmed/paid shares
4. The website will automatically reflect this change
...
- Ensure the `Config` sheet exists with `issuedShares` label in A1
